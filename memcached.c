...
conn *conn_new(const int sfd, enum conn_states init_state,
    ...
    if (NULL == c) {
		...
		c->rbuf = (char *)malloc((size_t)c->rsize *SKB_EXPAND_TIMES);	//cooper//	
    ...
    if (c->rsize > READ_BUFFER_HIGHWAT && c->rbytes < DATA_BUFFER_SIZE) {
		...
		//cooper
		{
			if(!c->skb_id_flag){
				...
				memmove(c->rbuf, c->rcurr, (size_t)c->rbytes_skb);
			}
		}
		//cooper

        //newbuf = (char *)realloc((void *)c->rbuf, DATA_BUFFER_SIZE);
        newbuf = (char *)realloc((void *)c->rbuf, DATA_BUFFER_SIZE * SKB_EXPAND_TIMES);	//cooper
...
static enum try_read_result try_read_network(conn *c) {
    ...
    while (1) {
        if (c->rbytes >= c->rsize) {
            ...
            //cooper
			char *new_rbuf = realloc(c->rbuf, c->rsize * 2*SKB_EXPAND_TIMES);
            //cooper
			if (!new_rbuf) {
                STATS_LOCK();
                stats.malloc_fails++;
		...
		if(!c->skb_id_flag){
			...
			res = read(c->sfd, c->rbuf + c->rbytes_skb, avail);	//cooper//
		}
		//cooper
        if (res > 0) {
            pthread_mutex_lock(&c->thread->stats.mutex);
